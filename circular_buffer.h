...
public:
	...
	void push_back(const T & i) {
		if (_full) { throw "circular_buffer FULL"; }
	...
	T pop_back() {
		if (empty()) { throw "circular_buffer EMPTY"; }
...
private:
	hand_t clockwise(hand_t& hand) const { if (hand == _capacity - 1) { hand = 0; } else hand++; return hand; }
	hand_t anticlockwise(hand_t& hand) const { if (hand == 0) { hand = _capacity - 1; } else hand--; return hand; }
	std::size_t size() {
